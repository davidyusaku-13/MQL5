...
#property description "Multi-Trade Manager v1.4 - BE Buffer & Flexible Trade Count"
...
input double Fixed_Lot_Size = 0.04;        // Fixed lot size per trade
...
input double Breakeven_Buffer_Pips = 5.0;  // Buffer above/below entry for breakeven (in pips)
...
double symbol_point;
double symbol_pip_size;  // Actual pip size (point * 10 for standard digits)
int symbol_digits;
...
string edit_be_buffer_name = "edit_be_buffer";
string label_be_level_name = "label_be_level";
...
int OnInit()
{
   ...
   // Standard digits (2,4): 1 pip = 10 points (e.g., XAUUSD: 0.01 point → 0.10 pip)
   // Fractional digits (3,5): 1 pip = 10 points (e.g., EURUSD: 0.00001 point → 0.0001 pip)
   if(symbol_digits == 2 || symbol_digits == 4)
      symbol_pip_size = symbol_point * 10.0;  // XAUUSD (2 digits), USDJPY (3→4 digits)
   else if(symbol_digits == 3 || symbol_digits == 5)
      symbol_pip_size = symbol_point * 10.0;  // EURUSD (5 digits), XAUUSD (3 digits)
   else if(symbol_digits == 1)
      symbol_pip_size = symbol_point * 10.0;  // BTCUSD sometimes (1 digit)
   else
      symbol_pip_size = symbol_point;  // Fallback for unusual digit counts
   ...
   Print("=== MultiTradeManager EA v1.4 Initialized ===");
   Print("Symbol: ", current_symbol);
   Print("Digits: ", symbol_digits, " | Point: ", symbol_point, " | Pip Size: ", symbol_pip_size);
   Print("1 pip = ", DoubleToString(symbol_pip_size, symbol_digits));
...
void OnDeinit(const int reason)
{
   ...
   DeletePanel();
   Print("MultiTradeManager EA v1.4 deinitialized");
}
...
void OnTick()
{
   ...
   //--- Update prices EVERY TICK (fast operation, no performance impact)
   double new_bid = SymbolInfoDouble(current_symbol, SYMBOL_BID);
   double new_ask = SymbolInfoDouble(current_symbol, SYMBOL_ASK);

   // Track if prices changed for conditional operations
   bool prices_changed = (new_bid != last_bid || new_ask != last_ask);
   ...
   //--- GUI updates every 100ms (responsive but smooth)
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   else if(id == CHARTEVENT_OBJECT_ENDEDIT)
   {
      ...
      else if(sparam == OName(edit_be_buffer_name) || sparam == OName(edit_open_price_name))
      {
         // Update BE level display when buffer or open price changes
         UpdateBELevelDisplay();
         // Update $ amounts when open price changes (affects loss/profit calculations in pending mode)
         if(sparam == OName(edit_open_price_name))
            UpdateLossProfitDisplay();
      }
      else if(StringFind(sparam, OName("edit_tp_")) >= 0 || StringFind(sparam, "edit_tp_") >= 0)
      {
         UpdateLossProfitDisplay();
         UpdateBELevelDisplay(); // Update BE level when TP changes (for cap validation)
      }
   }
}
...
bool CreatePanel()
{
   ...
   //--- Title label
   CreateLabel("label_title", "Multi-Trade Manager v1.4", ScalePos(10), ScalePos(10), clrDarkBlue, title_font_size);
   ...
   //--- BE Buffer
   CreateLabel("label_be_buffer", "BE Buffer:", ScalePos(10), ScalePos(184), clrBlack, font_size);
   CreateEdit(edit_be_buffer_name, DoubleToString(Breakeven_Buffer_Pips, 1), ScalePos(145), ScalePos(184), (int)(edit_width * 0.6), edit_height);
   CreateLabel("label_be_pips", "pips", ScalePos(145 + BASE_EDIT_WIDTH * 0.6 + 5), ScalePos(184), clrGray, font_size);

   //--- BE Level display (will be calculated dynamically)
   CreateLabel(label_be_level_name, "BE Level:", ScalePos(10), ScalePos(217), clrBlack, font_size - 1);
   CreateLabel("label_be_level_value", "---", ScalePos(145), ScalePos(217), clrDarkOrange, font_size - 1);

   //--- Execution type section
   CreateLabel("label_exec_type", "Execution:", ScalePos(10), ScalePos(250), clrBlack, font_size);
   CreateButton(btn_market_name, "MARKET", ScalePos(145), ScalePos(250), button_width, button_height, clrWhite);
   CreateButton(btn_pending_name, "PENDING", ScalePos(145 + BASE_BUTTON_WIDTH + 5), ScalePos(250), button_width, button_height, clrWhite);

   //--- Direction section
   CreateLabel("label_direction", "Direction:", ScalePos(10), ScalePos(283), clrBlack, font_size);
   CreateButton(btn_buy_name, "BUY", ScalePos(145), ScalePos(283), button_width, button_height, clrWhite);
   CreateButton(btn_sell_name, "SELL", ScalePos(145 + BASE_BUTTON_WIDTH + 5), ScalePos(283), button_width, button_height, clrWhite);

   //--- Number of trades
   CreateLabel("label_trades", "Trades:", ScalePos(10), ScalePos(316), clrBlack, font_size);
   CreateEdit(edit_trades_name, IntegerToString(Number_Of_Trades), ScalePos(145), ScalePos(316), edit_width, edit_height);
   CreateLabel("label_trades_note", "(any number)", ScalePos(145 + BASE_EDIT_WIDTH + 10), ScalePos(316), clrBlue, font_size - 1);

   //--- Open price (for pending orders)
   CreateLabel(label_open_price_name, "Open Price:", ScalePos(10), ScalePos(349), clrBlack, font_size);
   CreateEdit(edit_open_price_name, "0.00000", ScalePos(145), ScalePos(349), (int)(edit_width * 1.18), edit_height);

   //--- Stop Loss
   CreateLabel("label_sl", "Stop Loss Price:", ScalePos(10), ScalePos(382), clrBlack, font_size);
   CreateEdit(edit_sl_name, DoubleToString(Stop_Loss_Price, 5), ScalePos(145), ScalePos(382), (int)(edit_width * 1.18), edit_height);
   CreateLabel("label_sl_amount", "($0.00)", ScalePos(145 + BASE_EDIT_WIDTH + 20), ScalePos(382), clrRed, font_size - 1);

   //--- Take Profit fields for TP1 and TP2 only
   CreateLabel("label_tp_header", "Take Profit Levels:", ScalePos(10), ScalePos(415), clrDarkBlue, font_size);
   int tp_y_offset = 438;
   int tp_spacing = 33;
...
void DeletePanel()
{
   ...
   string bases[] = {
      ...
      "label_be_buffer", edit_be_buffer_name, "label_be_pips",
      label_be_level_name, "label_be_level_value",
...
//+------------------------------------------------------------------+
//| Update BE Level Display                                          |
//+------------------------------------------------------------------+
void UpdateBELevelDisplay()
{
   // Get current direction and prices
   if(current_bid <= 0 || current_ask <= 0)
   {
      ObjectSetString(0, OName("label_be_level_value"), OBJPROP_TEXT, "---");
      return;
   }

   // Determine entry price based on execution type and direction
   double entry_price = 0;

   if(selected_execution == EXEC_PENDING)
   {
      string open_price_text = ObjectGetString(0, OName(edit_open_price_name), OBJPROP_TEXT);
      entry_price = StringToDouble(open_price_text);
   }
   else
   {
      entry_price = (selected_direction == TRADE_BUY) ? current_ask : current_bid;
   }
   ...
   // Get TP1 for validation
   string tp1_text = ObjectGetString(0, OName("edit_tp_1"), OBJPROP_TEXT);
   double tp1_price = StringToDouble(tp1_text);

   // Get buffer
   string buffer_text = ObjectGetString(0, OName(edit_be_buffer_name), OBJPROP_TEXT);
   double buffer_pips = StringToDouble(buffer_text);

   // Calculate BE price with buffer
   ENUM_POSITION_TYPE pos_type = (selected_direction == TRADE_BUY) ? POSITION_TYPE_BUY : POSITION_TYPE_SELL;
   double be_level = CalculateBEWithBuffer(entry_price, pos_type, tp1_price);

   // Display
   if(be_level > 0)
   {
      ObjectSetString(0, OName("label_be_level_value"), OBJPROP_TEXT, DoubleToString(be_level, symbol_digits));
   }
   else
   {
      ObjectSetString(0, OName("label_be_level_value"), OBJPROP_TEXT, "---");
...
//+------------------------------------------------------------------+
//| Validate and Calculate BE Buffer                                 |
//+------------------------------------------------------------------+
double CalculateBEWithBuffer(double entry_price, ENUM_POSITION_TYPE pos_type, double tp1_price)
{
   ...
   // Calculate BE price with buffer (using pip size, not point)
   ...
   if(first_calc)
   {
      Print("[BE Buffer Calc] Entry: ", entry_price, " | Buffer: ", buffer_pips, " pips (",
            DoubleToString(buffer_in_price, symbol_digits), ") | BE: ", be_price);
      first_calc = false;
   }
